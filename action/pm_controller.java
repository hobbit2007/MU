...
public class pm_controller {
	...
	@FXML
	TableColumn<hmmr_pm_model, String> col_id_pm, col_ninst_pm, col_shop_pm, col_group_pm, col_lm_pm, col_os_pm, col_equip_pm, col_group_eq, col_pmn_pm, col_pmc_pm,
									   col_pmtype_pm, col_ool_pm, col_otv_pm, col_days_exp;
	
	@FXML
	JFXButton add_ap_pm, add_pm, upd_pm, del_pm, close_pm, upd_table_pm;
	
	public static String _id_pm, _ninst_pm_upd, _shop_pm_upd, _group_eq_upd, _lm_pm_upd, _os_pm_upd, _equip_pm_upd, _group_pm_upd, _pmn_pm_upd, _pmc_pm_upd,
						 _pmtype_pm_upd, _ool_pm_upd, _otv, _days_exp_upd;
	...
	public void initialize()
	{
		...
		col_shop_pm.setCellValueFactory(CellData -> CellData.getValue().ShopProperty());
	      ...
	      //Клик мышью внутри таблицы активирует кнопки Обновить и Удалить
	        ...
	        add_ap_pm.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent event) {
					...
					hmmr_pm_model _ccl = table_pm.getSelectionModel().getSelectedItem();
					String sql_rez = qr._select_for_pmplan(_ccl.getId());
					
					String pereodic = scl.parser_sql_str(sql_rez, 0);
					String b_date = scl.parser_sql_str(sql_rez, 1);
					String e_date = scl.parser_sql_str(sql_rez, 2);
					@SuppressWarnings("unused")
					String shop = scl.parser_sql_str(sql_rez, 3);
					Otv_for_task = scl.parser_sql_str(sql_rez, 4);
					...
					for (int i = 0; i < _general; i++) {
						...
						qr._insert_pm_year(_ccl.getId(), days, Otv_for_task);
						_count = _cnt + _count;
					}
	...
	private void func_upd(String str)
	{
		...
		_shop_pm_upd = scl.parser_sql_str(_sql_rez, 1);
		_group_pm_upd = scl.parser_sql_str(_sql_rez, 2);
		_lm_pm_upd = scl.parser_sql_str(_sql_rez, 3);
		_os_pm_upd = scl.parser_sql_str(_sql_rez, 4); 
		_equip_pm_upd = scl.parser_sql_str(_sql_rez, 5);
		_group_eq_upd = scl.parser_sql_str(_sql_rez, 6);
		_pmn_pm_upd = scl.parser_sql_str(_sql_rez, 7);
		_pmc_pm_upd = scl.parser_sql_str(_sql_rez, 8);
		_pmtype_pm_upd = scl.parser_sql_str(_sql_rez, 9);
		_ool_pm_upd = scl.parser_sql_str(_sql_rez, 10);
		_otv = scl.parser_sql_str(_sql_rez, 11);
		_days_exp_upd = scl.parser_sql_str(_sql_rez, 12);
	...
	private void lang_fun(String loc1, String loc2)
	{
		...
		col_shop_pm.setText(lngBndl.getString("col_shop_pm"));
		col_group_pm.setText(lngBndl.getString("col_group_pm"));
		col_lm_pm.setText(lngBndl.getString("col_lm_pm"));
		col_os_pm.setText(lngBndl.getString("col_os_pm"));
		col_equip_pm.setText(lngBndl.getString("col_equip_pm"));
		col_group_eq.setText(lngBndl.getString("col_group_eq"));
		col_pmn_pm.setText(lngBndl.getString("col_pmn_pm"));
		col_pmc_pm.setText(lngBndl.getString("col_pmc_pm"));
		col_pmtype_pm.setText(lngBndl.getString("col_pmtype_pm"));
