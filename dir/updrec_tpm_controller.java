...
public class updrec_tpm_controller
{
	...
	static String pathToPdf = "\\\\192.168.100.245\\MU\\Img";
	...
	private void initialize()
	{
		...
		upd_tpm.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				if(name_tpm.getText().length() != 0 && desc_tpm.getText().length() != 0)
				{
					...
					qr._insert_history(conn_connector.USER_ID, apwr_controller.USER_S + " - ќбновил запись є = " + type_pm_controller._id_tpm + " в таблице Activity Type");
	    ...
	    fc.setTitle("”кажите путь к файлу:");
